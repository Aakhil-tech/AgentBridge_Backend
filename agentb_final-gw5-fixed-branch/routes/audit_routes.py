...
Clean MVP endpoints — frontend-friendly responses
"""
...
# 🔥 CORE TRANSFORM (THIS FIXES EVERYTHING)
def transform(log):
    return {
        "decision": (log.get("verdict") or "").upper(),
        "risk": (log.get("risk_level") or "").upper(),
        "reason": log.get("reasoning") or "No explanation available",
        "next_step": (
            "Send to human review"
            if (log.get("verdict") or "").lower() == "review"
            else None
        )
    }


# ✅ LOGS (MAIN DASHBOARD DATA)
@router.get("/logs")
async def get_logs(api_key: str):
    ...
    logs = result.data or []
    return [transform(log) for log in logs]


# ✅ INCIDENTS (ONLY FLAGGED)
...
# ⚠️ KEEP REPORT AS-IS (not used in MVP UI)
@router.get("/report")
